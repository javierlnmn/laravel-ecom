    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div>
                    <p class="text-sm mt-2 text-zinc-800">
                        ...
                        <button form="send-verification" class="underline text-sm text-zinc-600 hover:text-zinc-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500">
        ...
        <div class="flex items-center gap-4">
            ...
            @if (session('status') === 'profile-updated')
                <p
                    x-breeze.data="{ show: true }"
                    x-breeze.show="show"
                    x-breeze.transition
                    x-breeze.init="setTimeout(() => show = false, 2000)"
