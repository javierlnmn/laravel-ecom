...
>
    ...
    >
        @foreach ($product->images as $image)
            <img class="transition-shadow duration-[500ms] cursor-pointer @if(isset($responsive)) w-full h-auto @else w-auto h-full @endif max-lg:w-auto max-lg:h-full aspect-square object-cover @if ($loop->first) shadow-lg @endif"  src="{{ $image->image }}" alt="{{ $image->alt }}">
        @endforeach
    </div>
