<div class="relative my-0 mx-auto mb-5">
    <div class="flex overflow-hidden snap-x snap-mandatory scroll-smooth">
        @foreach ($imageList as $image)
            <img class="flex-grow flex-shrink-0 basis-full snap-start aspect-square object-contain" id="slider-image-{{ $image->id }}" src="{{ $image->image }}" alt="{{ $image->alt }}">
        @endforeach
    </div>
        ...
        <div class="h-full relative"
            ...
            @foreach ($imageList as $image)
                <li class="absolute bottom-0 top-0 w-full h-full">
                    <img class="w-full h-full object-contain aspect-square" src="{{ $image->image }}" alt="">
                </li>
            @endforeach
            </ul>
        </div>
